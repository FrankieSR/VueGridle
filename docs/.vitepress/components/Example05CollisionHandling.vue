    ...
    <div class="main-content">
        ...
        <div class="console-panel">
            <div class="console-header">
                <button class="clear-btn" type="button" @click="collisionLogs = []">Clear</button>
            </div>
            <div class="console-body">
                <div v-if="collisionLogs.length === 0" class="console-line">
                    Move an item over another one.
                </div>
                <div v-for="(log, index) in collisionLogs" :key="index" class="console-line console-warning">
                    {{ log }}
                </div>
            </div>
        </div>
...
.grid-item-content.near {
    box-shadow: inset 0 0 0 2px var(--resize-handle-bg-color);
}
</style>
