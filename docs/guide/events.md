...
| Event    | Payload | Description                                                      |
| -------- | ------- | ---------------------------------------------------------------- |
| _(None)_ | -       | Currently, the `Grid` component does not emit any custom events. |
...
- **Example**:
    ```vue
    <GridItem
        @resize-stop="(x, y, w, h) => onResizeStop('item-1', x, y, w, h)"
        nodeId="item-1"
        resizable
    />
    ```
...
- **Example**:
    ...
    <GridItem
        v-model="item.grid"
        @update:model-value="onUpdateModelValue('item-1', $event)"
        nodeId="item-1"
    />
    ```
    ```ts
...
- **Description**: Fired when the item overlaps with other items in the grid, providing an array of colliding item IDs. Uses the parent `Grid` layout by default. The `GridItem` `allNodes` override is deprecated and remains available only for legacy or advanced custom layouts.
- **Example**:
    ```vue
    <Grid :layout="layout">
      <GridItem @collision-detected="onCollisionDetected('item-1', $event)" nodeId="item-1" />
    </Grid>
    ```
    ```ts
