...
| `layout`       | `GridNode[]` | `[]`    | Grid items shared with child items for collision and proximity. |
...
- **Type**: `GridNode[]`
- **Default**: `[]`
- **Description**: Provides the grid's item list to child `GridItem` components. When this is set, each child item can use collision detection without passing `allNodes` individually.
- **Example**:
    ```vue
...
- **Type**: `GridNode[]`
- **Default**: `undefined`
- **Deprecated**: Yes. Prefer passing the full layout once to the parent `Grid` via `layout`.
- **Description**: Legacy optional override for collision detection. Use `allNodes` only when a specific item needs to check against a different node list.
- **Example**:
    ```vue
    <Grid :layout="layout">
      <GridItem nodeId="item-1" v-model="layout[0].grid" />
    </Grid>
    ```
    ```ts
    const layout = [
